...
import com.acuityx.memorygame.utils.*
import com.google.firebase.firestore.ktx.firestore
import com.google.firebase.ktx.Firebase
import com.google.firebase.storage.ktx.storage
import java.io.ByteArrayOutputStream

class CreateNewBoardActivity : AppCompatActivity() {
    companion object {
        private const val TAG = "CreateNewBoardActivity"
        private const val PICK_PHOTO_CODE = 124
        ...
        adapter = ImagePickerAdapter(
            ...
            object : ImagePickerAdapter.ImageClickListener {
                override fun onPlaceholderClicked() {
                    if (isPermissionGranted(this@CreateNewBoardActivity, READ_PHOTOS_PERMISSION)) {
                        launchIntentForPhotos()
                    } else {
                        requestPermission(
                            this@CreateNewBoardActivity,
    ...
    override fun onRequestPermissionsResult(
        ...
        if (requestCode == READ_EXTERNAL_PHOTOS_CODE) {
            if (grantResults.isNotEmpty() && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                ...
                Toast.makeText(
                    this,
                    "The app needs access to photos to create a custom game",
                    Toast.LENGTH_LONG
                ).show()
            }
        }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode != PICK_PHOTO_CODE || resultCode != Activity.RESULT_OK || data == null) {
            Log.w(TAG, "No data retrieved from launched activity, flow likely canceled")
            return
        }
        ...
        for ((index, photoUri) in chosenImageUris.withIndex()) {
            ...
            photoReference.putBytes(imageByteArray)
                .continueWithTask { photoUploadTask ->
                    ...
                    Log.i(
                        TAG,
                        "Finished uploading $photoUri, total uploaded ${uploadedImageUrls.size}/${chosenImageUris.size}"
                    )
